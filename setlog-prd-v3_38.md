# Setlog Mobile Mini — PRD v3.38
> AI 역방향 워크플로우 기준 명세서 | 이 문서가 Single Source of Truth
> v3.37 → v3.38: **버그 수정 — 접근성/성능/보안 검증.** (1) 접근성: 아이콘만 있고 텍스트가 없는 터치 요소(친구 삭제/촬영시뮬 아이콘, 뮤트 버튼, 반응 추가 버튼, 탭바, 뒤로가기 버튼) 전부에 accessibilityLabel/Role 추가 — 이전엔 41개 터치 요소 중 접근성 속성이 0개였음. (2) 성능: TVNoise가 16ms(60fps)마다 600셀 그리드를 통째로 리렌더하고 있어 친구 3명 동시 waiting 시 최대 1800 View/tick 리렌더 우려 — 50ms(20fps)로 완화(시각적 차이 미미, 리렌더 부하 약 1/3). (3) 보안: accounts의 password가 평문 저장되는 것을 확인했으나, 네트워크·디스크 저장이 없는 로컬 시뮬레이션이라 실유출 경로가 없고 클라이언트 단독 해싱은 실질적 보안효과가 없어 가짜로 고치지 않고 "알려진 한계"로 정직하게 문서화. 섹션3에 접근성 원칙(8번) 추가, 체크리스트 2개 보강.
...
| 버전 | 3.38 |
| 기술 스택 | React Native (Expo) + StyleSheet + @expo/vector-icons (Ionicons) + expo-camera + expo-video + **react-native-svg** |
| 산출물 | App.js 단일 파일 |
...
| BorderBeam | ❌ | ✅ 테두리 순환 | ❌ | ❌ |
| CaptionLabel | ❌ | ❌ | ❌ | ✅ (≠null, 좌하단 레이어5, fs16, 1줄) |
